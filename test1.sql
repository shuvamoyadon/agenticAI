...
USING (
  ...
  /* =====================================================
     SOURCE + JSON FLATTENING
     ===================================================== */
  WITH exploded AS (
    SELECT
      line_of_business AS business_unit_cd,        -- Derived as per instruction
      ID,
      Product_Association_ID,
      CreateDate,                                  -- Used for orig_src_post_dts
      LastModifiedDate,                            -- Used for source_last_process_dts
...
THEN
  UPDATE SET
    ...
    T.record_status_cd = 'Inactive',               -- Added as per instruction
    T.record_end_dts = CURRENT_TIMESTAMP(),        -- SCD2 end dating
...
THEN
  ...
  VALUES (
    GENERATE_UUID(),                                -- Unique surrogate key
    'PCT',                                          -- Hardcoded source_system_cd
    ...
    S.reporting_detail,
    CONCAT(S.sf_product_configuration_id, '-', S.sf_product_association_id), -- business_hash_key_id
    TO_HEX(SHA256(S.sf_product_configuration_id)),   -- record_hash_key_id
    S.CreateDate,                                    -- orig_src_post_dts
    S.LastModifiedDate,                              -- source_last_process_dts
    CURRENT_TIMESTAMP(),                             -- record_start_dts
    TIMESTAMP('9999-12-31 00:00:00 UTC'),             -- record_end_dts
    'Active',                                        -- record_status_cd
    'Y',
    CURRENT_TIMESTAMP(),
    CURRENT_TIMESTAMP(),
    'PCT GCP Integration User'                       -- Hardcoded updated_by
  );
